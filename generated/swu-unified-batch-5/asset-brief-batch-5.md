...
Context: Existing style lives in project "swu-building-icons-batch-legacy-refresh" and "swu-unified-asset-icons". Match: dark sci-fi strategy UI, clean isometric/3D icon language, centered subject, square icon cells, high readability at 48px, rim lighting, subtle holographic blue/amber accents, transparent-safe dark neutral background. No text in icons, no logos, no protected actor likenesses. Output source contact sheets plus manifest.

Output contract:
- Generate 256x256-ready building icons as contact-sheet PNGs.
- Use one icon per family below; IDs in family share same icon.
- Split into sequential sheets if needed, e.g. batch-5-sheet-1.png, batch-5-sheet-2.png, ...; use manageable grids (max ~25 cells/sheet).
- Create generated-icons-batch-5.json with items: kind="building", label, slug, ids, contactSheet, cellIndex, short visualPrompt.
